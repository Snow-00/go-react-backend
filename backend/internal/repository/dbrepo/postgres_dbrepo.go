...
func (m *PostgresDBRepo) AllMovies(genreID ...int) ([]*models.Movie, error) {
...
func (m *PostgresDBRepo) OneMovieForEdit(id int) (*models.Movie, []*models.Genre, error) {
	...
	query = `SELECT g.id, g.genre 
			FROM movies_genres mg LEFT JOIN genres g ON mg.genre_id = g.id
...
func (m *PostgresDBRepo) UpdateMovie(movie models.Movie) error {
	...
	stmt := `UPDATE 
			movies SET title = $1, description = $2, release_date = $3, runtime = $4, mpaa_rating = $5,
